...
async function startServer() {
  ...
  app.get("/api/weather", async (req, res) => {
    ...
    if (isKeyMissing) {
      // Return high-quality mock weather data for demonstration so the app is immediately usable
...
function getMockWeatherData(query: string) {
  ...
  const mockHours = Array.from({ length: 8 }, (_, idx) => {
    ...
    let hTemp = tempC + Math.sin((hour - 12) / 6) * 4;
    let hCondText = conditionText;
...
function mapKoreanQueryToEnglish(query: string): { apiQuery: string; displayName: string } {
...
function applyLocalJejuAdjustments(data: any, rawQuery: string): any {
